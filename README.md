...
Version 0.2.4 makes diagnostics Mira's development mode. Launching `Start-Diagnostics.cmd`, starting `MiraDesk.exe --diagnostics`, or enabling diagnostics in the app exposes the Assistant and Dashboard to screen-capture tools and keeps them in Mira's own screenshots. Stopping diagnostics or allowing the 30-minute session to expire restores capture protection. A normal launch remains excluded from capture.

Version 0.2.3 fixes microphone capture in Hybrid Listen on Windows WebView2. Mira now resolves the selected native Windows endpoint to the browser's own input-device ID by label, requests a mono stream, and uses the WebView-compatible script processor for VAD. The microphone indicator turns green only after real audio frames arrive. A four-second watchdog reports a stalled stream instead of leaving a false active state. Diagnostic schema 3 adds the stream, track, sample-count, format, device-resolution, and processor metadata needed to distinguish hardware, device-selection, and WebView audio failures without exposing audio or transcript content.

Version 0.2.2 avoids the Windows capture-exclusion regression caused by hiding and showing an already-protected window. Changing **Hide Icon from Dock/Taskbar** now updates the native utility-window style and taskbar registration without cycling window visibility. When a real hide is required, Mira clears capture affinity before hiding and reapplies `WDA_EXCLUDEFROMCAPTURE` only after the window is shown again. Hidden Dashboard windows are protected after they become visible rather than while they are being created.
